        ...
        >
          <Card className="h-full border border-border bg-white hover:shadow-lg transition-shadow overflow-hidden">
            ...
            {(() => {
              ...
              if (project.cover) {
                const cover = project.cover as any; // Type assertion pour gérer différentes structures
                ...
                // Cas 2: cover.attributes.url (si pas de data)
                  ...
                  imageAlt = cover.attributes.alternativeText;
                }
                // Cas 3: cover.url (structure plate)
                else if (typeof cover === 'object' && 'url' in cover && typeof cover.url === 'string') {
                  imageUrl = cover.url;
